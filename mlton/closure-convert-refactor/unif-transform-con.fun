 ...
 * representations.
 *)
functor UnifTransformCon (S: TRANSFORM_STRUCTS): TRANSFORM =
struct
   ...
   let
      ...
      and loopBind {var = res, ty = resTy, exp}: unit =
         (case exp of
                ...
                let
                   ...
                   val newCases = 
                      (case cases of
                         ...
                         let
                            val newCases = Vector.keepAllMap (cases, fn (Sxml.Pat.T {con, arg, targs}, e) =>
                                if caseUsed {res=res, con=con}
                                   then SOME (Sxml.Pat.T {con = con,
                                              arg = arg,
                                              targs = targs}, e)
                                   else NONE)
                           val _ =
                               Vector.foreach
                               (newCases, fn (Sxml.Pat.T {con, arg, ...}, _) =>
                                case (conArgValue con, arg) of
                                   (NONE, NONE) => ()
                                 | (SOME v, SOME (x, _)) => AbsVal.unify (v, varValue x)
                                 | _ => Error.bug "UnifTransform.loopBind: Case")
                         in
                            Sxml.Cases.Con newCases
                         end
                      | _ => cases)
                   val _ =
                      Sxml.Cases.foreach
           ...
           | Sxml.PrimExp.Handle {try, handler, catch, ...} =>
                ...
                then
                   ...
                   AbsVal.unify (loopExp try, varValue res)
         ...
         (case dec of
             Sxml.Dec.MonoVal {var, ty, exp} =>
                let
                   ...
                   val isConApp = case exp of Sxml.PrimExp.ConApp _ => true
                                            | _ => false
                   val (cvar, cty) = convertBoundVar (var, ty)
                   val cdecs =
                      case (known, isConApp) of
                         (SOME {con, arg}, false) => 
...
in
fun scan _ = str "uniftranscon" *> str "(" *>
